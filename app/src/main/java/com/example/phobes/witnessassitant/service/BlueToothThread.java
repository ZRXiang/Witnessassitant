...
public class BlueToothThread extends Thread {
    private String address = "00:66:4B:48:2A:53";
    ...
    public BlueToothThread(Handler handlerin, Handler handlerout, Context context,String orderId) {
        inHandler = handlerin;
       ...
       /* try {
            LogWriter.open(CommData.filePath).print(DEBUG,"创建线程BlueToothThread");
        } catch (IOException e) {
            e.printStackTrace();
        }*/
    }
    ...
    public void connect() {
        ...
        try {
            initdate();
            //Log.i(TAG, "连接中……");
           // LogWriter.open(CommData.filePath).print(DEBUG,"BlueToothThread开始连接");
            client = mDevice.createRfcommSocketToServiceRecord(UUID.fromString("00001101-0000-1000-8000-00805F9B34FB"));
            try{
                client.connect();
            }catch(IOException e){
              //  LogWriter.open(CommData.filePath).print(ERROR,"BlueToothThread连接异常");
                e.printStackTrace();
            }
            //LogWriter.open(CommData.filePath).print(DEBUG,"BlueToothThread连接成功");
            //client.setSoTimeout(timeout);// 设置阻塞时间
            //Log.i(TAG, "连接成功");
            //LogWriter.log(DEBUG,"开始获取输入输出流");
            in = new DataInputStream(new BufferedInputStream(client.getInputStream()));
            out = new DataOutputStream(new BufferedOutputStream(client.getOutputStream()));
           // LogWriter.open(CommData.filePath).print(DEBUG,"输入输出流获取成功");
           // Log.i(TAG, "输入输出流获取成功");
    ...
    public void initdate() {
        ...
        address = sp.getString("mac_address", address);
        mBluetoothAdapter = BluetoothAdapter.getDefaultAdapter();
        mDevice = mBluetoothAdapter.getRemoteDevice(address);
        //Log.i(TAG, "获取到MAC地址:" + address );
        /*try {
         //   LogWriter.open(CommData.filePath).print(DEBUG,"获取到MAC地址成功:" + address);
        } catch (IOException e) {
            e.printStackTrace();
        }*/
    }
    ...
    public void run() {
       ...
       /* try {
            LogWriter.open(CommData.filePath).print(DEBUG,TAG+"开始运行");
        } catch (IOException e) {
            e.printStackTrace();
        }*/
        ...
        //Log.i(TAG, "1.run开始");
       /* try {
            LogWriter.open(CommData.filePath).print(DEBUG,TAG+"1.run开始");
        } catch (IOException e) {
            e.printStackTrace();
        }*/
        ...
        while (isRun) {
            try {
                if (client != null) {
                   // Log.i(TAG, "2.接收数据头");
                   // LogWriter.open(CommData.filePath).print(DEBUG,TAG+"2.接收数据头开始");
                    //循环读取头
                    while(true) {
                        in.read(buff, 0, 1);
                        if (buff[0]==71){//G
                    ...
                    //LogWriter.open(CommData.filePath).print(DEBUG,TAG+"2.接收数据头成功");
                    //判断tag 1 参数包 2 数据包 4 结束报 5 确认包
                    if(buff[4]==1)//head tag
                    {
                       // LogWriter.open(CommData.filePath).print(DEBUG,"接收参数包开始");
                        isRead(buff,5,16,in);
                        //判断当前的包头是否是要接收的数据包头
                        ...
                        */
                        ...
                        out.write(confirm);
                        out.flush();
                       // LogWriter.open(CommData.filePath).print(DEBUG,"接收参数包完成");
                       ...
                       // LogWriter.open(CommData.filePath).print(DEBUG,"接收第"+G_nPackIndex+"数据包开始");
                        ...
                        */
                        ...
                        if(data.PackIndex!=G_nPackIndex) {
                            ...
                            out.write(confirm);
                            out.flush();
                           // LogWriter.open(CommData.filePath).print(DEBUG,"接收第"+G_nPackIndex+"数据包完成");
                            //System.out.println("out2 length=" + out.size());
                        }
                    }
                    else if(buff[4]==4) {//finish tag
                       // LogWriter.open(CommData.filePath).print(DEBUG,"接收结束包开始");
                        isRead(buff, 5, 12, in);//edit by wt
                        /*
                        JavaStruct.unpack(data, buff, ByteOrder.BIG_ENDIAN);
                        ...
                        out.write(confirm);
                        out.flush();
                       // LogWriter.open(CommData.filePath).print(DEBUG,"接收结束包完成");
                    ...
                    if (isFinish) {
                       ...
                       // LogWriter.open(CommData.filePath).print(DEBUG,"蓝牙连接开始断开");
                        close();
                       // LogWriter.open(CommData.filePath).print(DEBUG,"蓝牙连接断开成功");
                        SendMessage("接受完毕", nCountLength, (byte) 4, nCountLength);
                       // LogWriter.open(CommData.filePath).print(DEBUG,TAG+"1.run 退出");
                        //Log.i(TAG, "1.run 退出");
                        break;
                    }
                } else {
               ...
               /* try {
                  //  LogWriter.open(CommData.filePath).print(ERROR,"数据接收错误" + e.getMessage());
                } catch (IOException e1) {
                    e1.printStackTrace();
                }*/
    ...
    * */
    ...
    public void close()
    {
        ...
        {
           /* try {
                LogWriter.open(CommData.filePath).print(ERROR,"关闭连接异常" + e.getMessage());
            } catch (IOException e1) {
                e1.printStackTrace();
            }*/
           // Log.i(TAG, "close err");
