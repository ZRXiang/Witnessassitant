...
import com.zbl.server.DevMsg;
import com.zbl.server.G_Var;
import com.zbl.server.RecvDevMsgListener;
 ...
 */
public class PointItemActivity extends AppCompatActivity {
    Toolbar toolbar;
            ...
            switch (msg.what) {
                case 0:
                    ...
                    try {
                        ...
                        /**{"ACTION":"START","TYPE":"ZIT","STARTINDEX":0,"LEN":128}
                         ACTION			表示当前动作，分START、DATA和FINISH
                         TYPE			表示文件类型
                         STARTINDEX		表示当前接收到数据保存到文件中的偏移
                         LEN				表示当前接收到数据长度
                         */
                        String type= jsonObject.getString("TYPE");
                        if(type.equals("ZIT")){
                            String s= jsonObject.getString("ACTION");

                            if(s.equals("START")){
                                CommData.dbSqlite.updateSiteTestDataAndFileName(pPointId,json[0]);
                            }else if(s.equals("DATA")){
                                CommData.dbSqlite.updateSiteTestDataAndReceiveState(pPointId,0,0);
                            }else if(s.equals("FINISH")){
                                CommData.dbSqlite.updateSiteTestDataAndReceiveState(pPointId,1,0);
                                mProgressDialog.dismiss();
                            }
                        }
                        else if(type.equals("ZPW")){
                            String s= jsonObject.getString("ACTION");
                            if(s.equals("START")){
                                CommData.dbSqlite.updateSiteTestDataAndFileName(pPointId,json[0]);
                            }else if(s.equals("SC")||s.equals("TD")){
                                CommData.dbSqlite.updateSiteTestDataAndReceiveState(pPointId,0,0);
                            }else if(s.equals("FINISH")){
                                CommData.dbSqlite.updateSiteTestDataAndReceiveState(pPointId,1,0);
                                mProgressDialog.dismiss();
    ...
    private void addEvent(){
        btnFileStart.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(state==1){
                    Snackbar.make(btnFileStart, "文件已接收!", Snackbar.LENGTH_LONG).setAction("Action", null).show();
                    return;
                }
                G_Var.setRecvDevMsgListener(new RecvDevMsgListener() {
                    @Override
                    public void onRecvDevMsg(DevMsg msg) {
                        String filepath = msg.filepath;
                        String param = msg.param;
                        String textToShow = filepath + "~" + param;
                        Message msgToSend = new Message();
                        msgToSend.what = 0 ;
                        msgToSend.obj = textToShow;
                        mHandler.sendMessage(msgToSend);
                        LogWriter.log(CommData.INFO,"filepath = " + filepath + " param = " + param);
                       //  Toast toast = Toast.makeText(PointItemActivity.this, "filepath = " + filepath + " param = " + param, Toast.LENGTH_SHORT);
                       // toast.show();
                    }
               ...
               /* SharedPreferences set=getSharedPreferences("SP",0);
                ...
                mProgressDialog = new ProgressDialog(PointItemActivity.this);
        ...
        btnPointItemFinish.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                int state= CommData.dbSqlite.querySiteTestDataSendState(pPointId);
                if(state==2){
        ...
        btnMessageSet.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(PointItemActivity.this,SetMessageAcitivity.class);
    ...
    public void initView(){
        ...
        //btnMessageSet.setVisibility(View.GONE);
        if(toolbar!=null){
    ...
    public void startSocket() {
        try{
            socketThread = new SocketThread(mhandler, mhandlerSend, PointItemActivity.this,orderId);
            socketThread.start();
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    private void startBlueTooth(){
        try{
            BlueToothThread blueToothThread=new BlueToothThread(mhandler, mhandlerSend, PointItemActivity.this,orderId);
